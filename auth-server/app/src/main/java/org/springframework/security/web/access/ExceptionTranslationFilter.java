 ...
 * {@link org.springframework.security.web.access.AccessDeniedHandler}. By
 * default the filter will use
 * {@link org.springframework.security.web.access.AccessDeniedHandlerImpl}.
 * <p>
 * To use this filter, it is necessary to specify the following properties:
 * <ul>
 * <li><code>authenticationEntryPoint</code> indicates the handler that should
 ...
 * <li><tt>requestCache</tt> determines the strategy used to save a request
 * during the authentication process in order that it may be retrieved and
 * reused once the user has authenticated. The default implementation is
...
public class ExceptionTranslationFilter extends GenericFilterBean {
	...
	public void afterPropertiesSet() {
		...
		contentNegotiationStrategy = SpringContextUtils.getApplicationContext().getBean(ContentNegotiationStrategy.class);
	...
	protected void sendStartAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, AuthenticationException reason) throws ServletException, IOException {
		...
		if (requestMatcher.matches(request)) {
			...
			logger.debug("Calling Authentication entry point. 改变为sending 401 Unauthorized error");
	...
	private RequestMatcher createRequestMatcher() {
		...
		// boolean isCsrfEnabled = http.getConfigurer(CsrfConfigurer.class) !=
		// null;
