...
import com.neusoft.ehrss.liaoning.provider.ca.GXCA.product.config.GXCAProductWSClient;
...
@Profile({ "dev_cloud", "product" })
@Service
public class GXCAProductServiceImpl implements GXCAGatewayService {

	private static final Logger log = LoggerFactory.getLogger(GXCAProductServiceImpl.class);

	@Value("${saber.gxca.appName}")
	private String appName;

	@Resource
	private GXCAProductWSClient gxcaProductWSClient;
	...
	public boolean verifySignedData(String base64EncodeCert, String inData, String signValue) {
		...
		VerifySignedData2Response verifySignedDataResponse = gxcaProductWSClient.verifySignedData(appName, base64EncodeCert, inData, signValue);
		if (verifySignedDataResponse == null)
			throw new BusinessException("数据验签失败");
		log.debug("GXCA验证数据签名verifySignedDataResult={}", verifySignedDataResponse.isOut());
		if (verifySignedDataResponse.isOut()) {
			return true;
		}
		throw new BusinessException("数据验签失败");
	}
	...
	public boolean validateCert(String base64EncodeCert) {
		log.debug("GXCA验证证书");
		ValidateCertResponse validateCertResponse = gxcaProductWSClient.validateCert(appName, base64EncodeCert);
		if (validateCertResponse == null)
			throw new BusinessException("证书验证失败");
		log.debug("GXCA验证证书validateCert={}", validateCertResponse.getOut());
		int result = validateCertResponse.getOut();
		switch (result) {
		case 1:
			return true;
		case -1:
