package com.neusoft.sl.si.authserver.uaa.filter;
 ...
 * 
 * The implementation is based on guidance from: http://www.w3.org/TR/cors/
...
public class CorsFilter extends OncePerRequestFilter {
	...
	@Value("${cors.xhr.allowed.swagger2}")
	/**
	 * 启用aip-doc，为true； 启用项目，为false； 启用api-doc和项目，不可兼得
	 */
	...
	public void initialize() {
		...
		for (String allowedUri : this.corsXhrAllowedUris) {
			try {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug(String.format("URI '%s' allows 'X-Requested-With' header in CORS requests.", allowedUri));
				}
		...
		for (String allowedOrigin : this.corsXhrAllowedOrigins) {
			try {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug(String.format("Origin '%s' allowed 'X-Requested-With' header in CORS requests.", allowedOrigin));
				}
			} catch (PatternSyntaxException patternSyntaxException) {
...
//				if (corsXhrAllowedXRequestedWiths.contains(xRequestedWith)) {
					filterChain.doFilter(request, response);
					return;
//				}
//				else {
//					LOG.error("=====【X-Requested-With=" + xRequestedWith + "】======");
//				}
			}
	...
	static boolean isXhrRequest(final HttpServletRequest request) {
		...
		return StringUtils.isNotEmpty(xRequestedWith) || (StringUtils.isNotEmpty(accessControlRequestHeaders) && containsHeader(accessControlRequestHeaders, "X-Requested-With"));
	}
	...
	private boolean isCorsXhrAllowedRequestUri(final String uri) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(String.format("The '%s' URI does not allow CORS requests with the 'X-Requested-With' header.", uri));
		}
		return false;
	}
	...
	private boolean isCorsXhrAllowedOrigin(final String origin) {
		/**
		 * 正常项目，当origin为空时候，返回false。为了适应Swagger2，增加配置开关。
		 * 如果启用Swagger，则返回true，此时不可用于项目
		 */
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(String.format("The '%s' origin is not allowed to make CORS requests with the 'X-Requested-With' header.", origin));
		}
		return false;
	}
	...
	public List<String> getCorsXhrAllowedXRequestedWiths() {
		return corsXhrAllowedXRequestedWiths;
	}

	public void setCorsXhrAllowedXRequestedWiths(List<String> corsXhrAllowedXRequestedWiths) {
		this.corsXhrAllowedXRequestedWiths = corsXhrAllowedXRequestedWiths;
	}
